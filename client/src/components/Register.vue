...
<template>
  <div class="min-h-[80vh] flex items-center justify-center px-4 py-12 sm:px-6 lg:px-8">
    <div class="w-full max-w-md space-y-8 bg-white p-10 rounded-2xl shadow-xl border border-gray-50">
      <div>
        <div class="w-16 h-16 mx-auto bg-linear-to-tr from-purple-600 to-indigo-600 rounded-2xl flex items-center justify-center shadow-lg mb-6">
          <span class="text-2xl font-bold text-white">FS</span>
        </div>
        <h2 class="text-center text-3xl font-extrabold text-gray-900 tracking-tight">Créer un compte</h2>
        <p class="mt-2 text-center text-sm text-gray-500">
          Uploadez et partagez en privé
        </p>
      </div>
      ...
      <form class="mt-8 space-y-6" @submit.prevent="submit">
        ...
        <div class="text-center text-sm font-medium mt-4">
          ...
          <router-link to="/login" class="ml-1 text-indigo-600 hover:text-indigo-500 hover:underline transition-all">Se connecter</router-link>
        </div>
      </form>
    </div>
  </div>
